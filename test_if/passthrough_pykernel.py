...
eprint(f"vector size: {vector_size}")
eprint(f"line_size: {line_size}")
eprint(f"line_type: {line_type}")
eprint(f"vector_type: {vector_type}")

# Dataflow with ObjectFifos
of_in = ObjectFifo(line_type, name="in")
of_out = ObjectFifo(line_type, name="out")
of_out_odd = ObjectFifo(line_type, name="out_odd")
...
#val = np.array([np.uint8(32)], dtype=np.uint8)

def core_fn(of_in, of_out,of_out_odd):
    #tmp_buffer = LocalBuffer(
    #    type=np.ndarray[(1,), np.dtype[np.uint8]],
    #    initial_value=val,
    #)
    #m_idx = index.constant(32)
    #i_ui8 = index.casts(T.ui8(), m_idx)
    ...
    #for i in range_(line_size):
        #elemOut[i] = elemIn[i]
        #i_i32 = index.casts(T.i32(), i)
...
my_worker = Worker(core_fn, [of_in.cons(), of_out.prod(),of_out_odd.prod()])

# Runtime operations to move data to/from the AIE-array
rt = Runtime()
with rt.sequence(vector_type, vector_type,vector_type,vector_type) as (a_in, b_out,b_out_odd ,_):
    rt.start(my_worker)
    rt.fill(of_in.prod(), a_in)
    rt.drain(of_out_odd.cons(),b_out_odd,wait=False)
