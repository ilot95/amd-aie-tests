                ...
                for _ in range_(0xFFFFFFFF):
                    ...
                    for _ in range_(iters_outer):
                        ...
                        for _ in range_(iters_inner):
                            ...
                            join_cnt[0] =0
                            for y in range_(tile_ty_size_in):
                                for yy in range_(tile_ty_size_in):
                                    #firstout[y*tile_ty_size_in+yy] = elem_in[y] + elem_inner[yy]
                                    elem_in_i = elem_in[y]

                                    cmpjoin = arith.cmpi("eq", elem_in_i, elem_inner[yy])
                                    jcc = join_cnt[0]
                                    firstout[jcc] = arith.select(cmpjoin, elem_in_i , arith.constant(0, type=i32()))
                                    join_cnt[0] =  jcc + 1

                                    #This works but need dynamc object fifos
                                    # elem_in_i = elem_in[y]
                                    # with if_(elem_in_i == elem_inner[yy]):
                                    #     jcc = join_cnt[0]
                                    #     firstout[join_cnt[0]] = elem_in_i
                                    #     join_cnt[0] = jcc + 1
