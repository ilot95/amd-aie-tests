...
def external_mem_to_core():
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            tile_ty_size = 2048
            iters = host_elements // tile_ty_size
            ...
            def core_body_02():
                ...
                for _ in range_(0xFFFFFFFF):
                    for _ in range_(iters):
                        ...
                        elemOut_even = of_out1.acquire(ObjectFifoPort.Produce, 1)
                        elemOut_odd = of_out1_odd.acquire(ObjectFifoPort.Produce, 1)
                        call(odd_even, [elem_in, elemOut_odd, elemOut_even, tile_ty_size])
                        of_in1.release(ObjectFifoPort.Consume, 1)
                        of_out1_odd.release(ObjectFifoPort.Produce, 1)
                        of_out1.release(ObjectFifoPort.Produce, 1)
            ...
            def sequence(inTensor,outOddTensor, outTensor,):
                ...
                for i in range(transfers):
                    ...
                    out_task1 = shim_dma_single_bd_task(
                        of_out1, outTensor,offset= i *tranfer_size_elemnts , sizes=[1, 1, 1, tranfer_size_elemnts], issue_token=True
                    )
